// Partly modified 2011, 2012 Sony Ericsson Mobile Communications AB.
...
typedef struct {
    ...
    rat_t FocalLength;
    float ExposureTime;
    char ExposureTimeRational[24];  // theoretical max size: -2147483648/-2147483648 (11+1+11+1)
    char ExposureTimeJava[24];
    float ApertureFNumber;
    char ApertureFNumberRational[24];
    char ApertureFNumberJava[24];
    float Distance;
    float CCDWidth;
    char ExposureBias[24];
    char DigitalZoomRatio[24];
    ...
    char GpsAlt[20];
    rat_t GpsAltRaw;
    signed char GpsAltRef;
    // gps-datestamp is 11 bytes ascii in EXIF 2.2
    char GpsDateStamp[11];  // e.g. 2012-12-24 (11 character string)
    char GpsTimeStamp[11];  // e.g. 12:30:00 (three rational converted from file)
...
typedef struct {
    ...
    IFD_t IFD;          // the tag is related to an IFD e.g. IFD_BASE, IFD_EXIF, IFD_GPS
} ExifElement_t;
...
void ValidateJavaEntries(ExifElement_t* exifElementTable, int entryNum);
...
const char* GpsTagValToName(unsigned short tag);
