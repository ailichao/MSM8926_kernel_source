...
// Save data read now for later writing when saving attributes
typedef struct {
    int Tag;
    int Format;
    int Components;
...
typedef struct {
    NewElementAction_t NewElementAction;
    int UsedElements;
    int AllocatedElements;
    StorageUntilSaveElement_t* Storage;
} StorageUntilSave_t;

static StorageUntilSave_t StoreUntilSaved;
...
#ifdef SUPERDEBUG
static const char* TagValToName(int tag, IFD_t IFD)
{
    if (IFD == IFD_GPS) {
        return GpsTagValToName(tag);
    }
    unsigned int i;
    for (i = 0; i < TAG_TABLE_SIZE; i++) {
...
IFD_t getDefaultIFD(int tag)
{
    if (tag == TAG_EXPOSURETIME ||
        tag == TAG_FNUMBER ||
        tag == TAG_SHUTTERSPEED ||
        tag == TAG_EXPOSURE_PROGRAM ||
        tag == TAG_ISO_EQUIVALENT ||
        tag == TAG_EXIF_VERSION ||
        tag == TAG_DATETIME_ORIGINAL ||
        tag == TAG_DATETIME_DIGITIZED ||
        tag == TAG_COMPONENTS_CONFIG ||
        tag == TAG_CPRS_BITS_PER_PIXEL ||
        tag == TAG_EXPOSURE_BIAS ||
        tag == TAG_MAXAPERTURE ||
        tag == TAG_METERING_MODE ||
        tag == TAG_LIGHT_SOURCE ||
        tag == TAG_FLASH ||
        tag == TAG_FOCALLENGTH ||
        tag == TAG_EXPOSURE_MODE ||
        tag == TAG_WHITEBALANCE ||
        tag == TAG_DIGITALZOOMRATIO ||
        tag == TAG_SCENE_CAPTURE_TYPE ||
        tag == TAG_SUBJECT_DISTANCE ||
        tag == TAG_DISTANCE_RANGE ||
        tag == TAG_MAKER_NOTE ||
        tag == TAG_USERCOMMENT ||
        tag == TAG_FLASH_PIX_VERSION ||
        tag == TAG_COLOR_SPACE) {
        return IFD_EXIF;
    }

    if (tag == TAG_EXIF_IMAGEWIDTH ||
        tag == TAG_EXIF_IMAGELENGTH) {
        return IFD_EXTRA;
    }
...
static void StoreTagUntilSaved(int tag, int format, int components,
                               unsigned char *valuePtr, IFD_t IFD) {
    char outStr[50];            // used for static sized output
    char* outPtr = outStr;      // points to output element data (defaults to static sized output)
    char* outDynamic = NULL;    // used for dynamically allocated data
    int byteSize;
    ...
    switch (format) {
        ...
        case FMT_SLONG:
        {
            ...
            int i;
            int inPos = 0;
            outPtr = outDynamic = malloc(components * 12 + 1);  // alloc for maximum size of long
                                                                // in a string
            if (outDynamic == NULL) {
                ALOGE("StoreTagUntilSaved signed failed, allocsize %d", components * 12 + 1);
                goto cleanup;
            }
            byteSize = 0;
            for (i = 0; i < components; i++, inPos+= BytesPerFormat[format]) {
                if (i != 0) strcat(&outDynamic[byteSize++], ",");
                iVal = (int)ConvertAnyFormat(valuePtr+inPos, format);
                byteSize = byteSize + sprintf(&outDynamic[byteSize], "%d", iVal);
            }
            byteSize++; // include null terminator
            break;
        }
        ...
        case FMT_ULONG:
        {
            ...
            int i;
            int inPos = 0;
            outPtr = outDynamic = malloc(components * 11 + 1);  // alloc for maximum size of
                                                                // unsigned long in a string
            if (outDynamic == NULL) {
                ALOGE("StoreTagUntilSaved unsigned failed, allocsize %d", components * 11 + 1);
                goto cleanup;
            }
            byteSize = 0;
            for (i = 0; i < components; i++, inPos+= BytesPerFormat[format]) {
                if (i != 0) strcat(&outDynamic[byteSize++], ",");
                sVal = (unsigned int)ConvertAnyFormat(valuePtr+inPos, format);
                byteSize = byteSize + sprintf(&outDynamic[byteSize], "%u", sVal);
            }
            byteSize++; // include null terminator
            break;
        }
        ...
        case FMT_SRATIONAL:
        {
            ...
            num = Get32s(valuePtr);
            den = Get32s(4+(char *)valuePtr);
            byteSize = sprintf(outPtr, "%d/%d", num, den);
            byteSize++; // include null terminator
            break;
        }
        ...
        {
            double fVal;
            fVal = ConvertAnyFormat(valuePtr, format);
            byteSize = sprintf(outPtr, "%f", fVal);
            byteSize++; // include null terminator
            break;
        }
        ...
        case FMT_STRING:
            if (components <= 0) {
                components = strlen((char *)valuePtr) + 1; // include null terminator
            }
            outPtr = (char *)valuePtr;
            byteSize = components;
            break;
        ...
        default:
            goto cleanup;     // unknown type
    }

    if (StoreUntilSaved.UsedElements >= StoreUntilSaved.AllocatedElements) {
        ALOGE("StoreTagUntilSaved internal failure UsedElements %d, AllocatedElements %d",
              StoreUntilSaved.UsedElements, StoreUntilSaved.AllocatedElements);
        goto cleanup;
    }

    // store new data
    StorageUntilSaveElement_t* storage = &StoreUntilSaved.Storage[StoreUntilSaved.UsedElements];
    storage->Tag = tag;
    storage->Format = format;
    storage->Components = components;
    storage->IFD = IFD;
    storage->Value = malloc(byteSize);
    if (storage->Value == NULL) {
        ALOGE("StoreTagUntilSaved alloc element failed, allocsize %d", byteSize);
        goto cleanup;
    }
    memcpy(storage->Value, outPtr, byteSize);

    StoreUntilSaved.UsedElements++;
...
void ReportTagInfo(int tag, int format, int components,
                         unsigned char *valuePtr, IFD_t IFD) {

    if (StoreUntilSaved.UsedElements > 10000) {    // Sanity check
        ALOGE("ReportTagInfo Too many tags found: %d", StoreUntilSaved.UsedElements);
        return;
    }

    if (format > NUM_FORMATS || format <= 0) {      // unknown format
        ALOGE("ReportTagInfo Invalid format %d found for tag: %d", format, tag);
        return;
    }

    switch (StoreUntilSaved.NewElementAction) {
        case ACTION_COUNT:
            StoreUntilSaved.UsedElements++;
            break;
        case ACTION_SAVE:
            StoreTagUntilSaved(tag, format, components, valuePtr, IFD);
            break;
        case ACTION_IGNORE:
            break;
    }
...
#ifdef SUPERDEBUG
static void dumpList(const char *header, ExifElement_t* exifElementTable, int numElements) {
    printf("dumpList: %s", header);
    int i;
    for (i=0; i < numElements; i++) {
        unsigned short tag = exifElementTable[i].Tag;
        int DataLength = exifElementTable[i].DataLength;
        IFD_t IFD = exifElementTable[i].IFD;
        int format = exifElementTable[i].Format;
        const char *name = TagValToName(tag, IFD);
        printf("[%d] tag=0x%X name=%s, length=%d, IFD=%d, format=%d, data=%s",
             i, tag, name, DataLength, IFD, format, exifElementTable[i].Value);
    }
}
#endif

static int ReplacedValuesOk(char **ptr, char* newValue) {
    char* oldValue = *ptr;
    char *newPtr = malloc(strlen(newValue)+1);
    if (newPtr == NULL) return FALSE;
    strcpy(newPtr, newValue);
...
// After call to this function - just count the number of items
void StoreInitCountOnly() {
    memset(&StoreUntilSaved, 0, sizeof(StoreUntilSaved));
    StoreUntilSaved.NewElementAction = ACTION_COUNT;
}

// After call to this function - store all read entries
int StoreInitAllocate(int NumEntries, int hasDateTimeTag) {
    memset(&StoreUntilSaved, 0, sizeof(StoreUntilSaved));
    StoreUntilSaved.NewElementAction = ACTION_SAVE;
    if (!hasDateTimeTag) {
        // We have to write extra date time tag. The entry number should be
        // adjusted.
        ++NumEntries;
    }
    StoreUntilSaved.Storage = malloc(NumEntries * sizeof(StorageUntilSaveElement_t));
    if (StoreUntilSaved.Storage == NULL) return FALSE;
    StoreUntilSaved.AllocatedElements = NumEntries;
    return TRUE;
}

// After call to this function - ignore read entries
void StoreInitIgnore() {
    StoreUntilSaved.NewElementAction = ACTION_IGNORE;
}

int StoreGetUsedElements() {
    return StoreUntilSaved.UsedElements;
}

// Release store and from now ignore read entries
void StoreRelease() {
    if (StoreUntilSaved.Storage) {
        free(StoreUntilSaved.Storage);
    }
    memset(&StoreUntilSaved, 0, sizeof(StoreUntilSaved));
    StoreUntilSaved.NewElementAction = ACTION_IGNORE;
}

// Compare the tag of two elements  (used for sorting below)
static int cmpElemTag(const void* p1,const void* p2) {
    int tag1 = ((ExifElement_t*)p1)->Tag;
    int tag2 = ((ExifElement_t*)p2)->Tag;
    return tag1 - tag2; // tags are 16 bit, so no problem with underflow/overflow.
}

// Move already stored entries to the java entry list for later writing
void AddStoredEntriesToList(ExifElement_t* javaElements, int startIndex, int hasDateTimeTag) {
    ...
    if (!hasDateTimeTag) {
        ...
        ReportTagInfo(TAG_DATETIME, FMT_STRING, -1, (unsigned char *)dateBuf,
                      getDefaultIFD(TAG_DATETIME));

    }

    int toIndex = startIndex;
    int fromIndex = 0;
    for (; toIndex <  startIndex+StoreUntilSaved.UsedElements; fromIndex++, toIndex++) {
        StorageUntilSaveElement_t* from = &StoreUntilSaved.Storage[fromIndex];
        ExifElement_t* to = &javaElements[toIndex];
        to->Value = from->Value;
        to->DataLength = from->Components;
        to->Tag = from->Tag;
        to->Format = from->Format;
        to->IFD = from->IFD;
        from->Value = NULL;    // just clear, since copied above.
    }
    qsort(javaElements, startIndex+StoreUntilSaved.UsedElements, sizeof(ExifElement_t), cmpElemTag);
}

// Remove duplicates and replace unchanged java entries with original value format
void ValidateJavaEntries(ExifElement_t* javaElements, int entryNum) {
    int i, j;

    for (i = 0; i < entryNum; i++) {

        if (javaElements[i].IFD != IFD_GPS) {
            switch (javaElements[i].Tag) {
                case TAG_IMAGE_WIDTH:
                case TAG_IMAGE_LENGTH:
                    // Do not create new TAG_IMAGE_WIDTH or TAG_IMAGE_LENGTH
                    {
                        int found = FALSE;
                        for (j = 0; j < StoreUntilSaved.UsedElements; j++) {
                            if (javaElements[i].IFD == StoreUntilSaved.Storage[j].IFD &&
                                     javaElements[i].Tag == StoreUntilSaved.Storage[j].Tag) {
                                found = TRUE;
                                StoreUntilSaved.Storage[j].IFD = -1; // disable old item
                            }
                        }
                        if (!found) {
                            javaElements[i].IFD = -1; // disable new item
                        }
                    }
                    break;

                case TAG_EXPOSURETIME:
                    // check if exposure time not modified
                    if (strcmp(javaElements[i].Value, ImageInfo.ExposureTimeJava) == 0) {
                        // replace Java string with original rational string
                        if (!ReplacedValuesOk(&javaElements[i].Value,
                                               ImageInfo.ExposureTimeRational)) {
                            ALOGE("ExifValidateJavaEntries replace ExposureTime failed");
                        }
                    }
                    break;

                case TAG_FNUMBER:
                    // check if fnumber not modified
                   if (strcmp(javaElements[i].Value, ImageInfo.ApertureFNumberJava) == 0) {
                    // replace Java string with original rational string
                        if (!ReplacedValuesOk(&javaElements[i].Value,
                                               ImageInfo.ApertureFNumberRational)) {
                            ALOGE("ExifValidateJavaEntries replace FNumber failed");
                        }
                    }
                    break;
            }
...
// Convert a float to a rational string
// Value range limitations:
// A value is represented by a rational value.
// The following values can be representated:
// Values between 2147483647 and 1/2147483647, 0,
// and values between -1/2147483647 and -2147483647.
// The max number of significant digits is limited to 24 bits by float definition.
// However, for values with large denominator the significance of the numerator will be lower.
// This function uses denominators that are a power of 10 upto 1000 000 000.
static void FloatToRationalString(float inVal, char* toStr) {

    int negative = inVal < 0.0;
    double fromVal = fabsf(inVal);

    // check limits
    if (fromVal < 1.0E-9f) { // too small to fit in a Rational string
        strcpy(toStr, "0/1");
        return;
    } else if (fromVal >= 2.147483E+09) { // too large to fit in a Rational string
        sprintf(toStr, "%s2147483647/1", negative ? "-" : "");
        return;
    }

    // calculate exponent
    int exp;
    exp = (int)log10(fromVal);
    if (fromVal < 1.0) exp--;  // Truncated negative exponents adjusted

    // Use sprintf to format with rounding by using sprintf with precision argument.
    // Precision defines the number of digits to the right of the decimal point in the
    // formatting (in this case 0-5 digits, total number of digits 1-6).
    // Since largest denominator is 1000 000 000, the precision must be lowered for
    // smaller positive values, for example "12/1000 000 000" replaces 123/10 000 000 000.
    char floatStr[20];
    int precision = 5;
    if (exp <= -5) {
        precision = exp + 9;
    }
    sprintf(floatStr, "%.*E", precision, fromVal);

    // copy digits from mantissa
    unsigned int i, j;
    for (i = 0, j = 0; i < strlen(floatStr); i++) {
        if (isdigit(floatStr[i])) {
            toStr[j++] = floatStr[i];
        } else if (floatStr[i] == 'E') break;
    }
    toStr[j] = 0;

    // remove closing zeroes
    unsigned int digitNum = strlen(toStr);
    for (i = digitNum-1; i > 0 && toStr[i] == '0'; i--) {
        toStr[i] = 0;
        digitNum = i;
    }

    // generate rational string
    int denominator = digitNum - exp;
    if (denominator > 0) {
        strcat(toStr, "/1");
    ...
    // add minus character for negative values
    if (negative) {
        memmove(toStr+1, toStr, strlen(toStr)+1);
        toStr[0] = '-';
    }
}

static void saveExposureTimeForJava() {
    const char* format;
    ...
    snprintf(ImageInfo.ExposureTimeJava,
             sizeof(ImageInfo.ExposureTimeJava),
             format,
             ImageInfo.ExposureTime);
}

static void saveApertureFNumberForJava() {
    snprintf(ImageInfo.ApertureFNumberJava,
             sizeof(ImageInfo.ApertureFNumberJava),
             "%3.1f",
...
static void ProcessExifDir(unsigned char * DirStart, unsigned char * OffsetBase,
        unsigned ExifLength, int NestingLevel, int IFD)
{
    int de;
    int a;
    int NumDirEntries;
    ...
    if (ShowTags){
        printf("(dir has %d entries), level %d",NumDirEntries, NestingLevel);
    }
...
#ifdef SUPERDEBUG
        printf("element# %d tag 0x%02X (%s), Format %d (%s), Components %d, level %d, IFD %d",
             de, Tag, TagValToName(Tag, IFD), Format, formatStr(Format),
             Components, NestingLevel, IFD);
#endif
...
//                        printf("\"\n");
        ...
        switch(Tag){
            ...
            case TAG_DATETIME_DIGITIZED:
                ReportTagInfo(Tag, Format, Components, ValuePtr, IFD);
                // Fallthru...
            case TAG_DATETIME:
            ...
            case TAG_FNUMBER:
                ...
                // enable for java and save as rational string if not modified by java
                saveApertureFNumberForJava();
                {
                    unsigned num = Get32u(ValuePtr);
                    unsigned denom = Get32u(4+(char *)ValuePtr);
                    sprintf(ImageInfo.ApertureFNumberRational, "%d/%d", num, denom);
                }
                break;
            ...
            case TAG_MAXAPERTURE:
                ...
                if (ImageInfo.ApertureFNumber == 0){
                    ...
                    // enable for java and save as rational string if not modified by java
                    saveApertureFNumberForJava();
                    FloatToRationalString(ImageInfo.ApertureFNumber,
                                          ImageInfo.ApertureFNumberRational);
                } else {
                    ReportTagInfo(Tag, Format, Components, ValuePtr, IFD);
                }
            ...
            case TAG_EXPOSURETIME:
                ...
                // enable for java and save as rational string if not modified by java
                saveExposureTimeForJava();
                {
                    unsigned num = Get32u(ValuePtr);
                    unsigned denom = Get32u(4+(char *)ValuePtr);
                    sprintf(ImageInfo.ExposureTimeRational, "%d/%d", num, denom);
                }
                break;
            ...
            case TAG_SHUTTERSPEED:
                ...
                if (ImageInfo.ExposureTime == 0){
                    ...
                    // enable for java and save as rational string if not modified by java
                    saveExposureTimeForJava();
                    FloatToRationalString(ImageInfo.ExposureTime, ImageInfo.ExposureTimeRational);
                } else {
                    ReportTagInfo(Tag, Format, Components, ValuePtr, IFD);
                }
                break;
            ...
            case TAG_EXPOSURE_BIAS:
                {
                    unsigned num = Get32u(ValuePtr);
                    unsigned denom = Get32u(4+(char *)ValuePtr);
                    sprintf(ImageInfo.ExposureBias, "%d/%d", num, denom);
                }
                break;
            ...
            case TAG_DIGITALZOOMRATIO:
                    sprintf(ImageInfo.DigitalZoomRatio,
                    "%u/%u", Get32u(ValuePtr), Get32u(ValuePtr+4));
                break;
            ...
            case TAG_INTEROP_OFFSET:
                if (Tag == TAG_INTEROP_OFFSET && ShowTags) printf("%s    Interop Dir:",IndentString);
                {
                    ...
                    if (SubdirStart < OffsetBase || SubdirStart > OffsetBase+ExifLength){
                        ...
                        if (Tag == TAG_INTEROP_OFFSET) {
                            ProcessExifDir(SubdirStart, OffsetBase, ExifLength,
                                           NestingLevel+1, IFD_INTER_OP);
                        } else {
                            ProcessExifDir(SubdirStart, OffsetBase, ExifLength,
                                           NestingLevel+1, IFD_EXIF);
                        }
                    }
                    continue;
                }
...
void process_EXIF (unsigned char * ExifSection, unsigned int length)
{
    ...
    ProcessExifDir(ExifSection+8+FirstOffset, ExifSection+8, length-8, 0, IFD_BASE);
...
const TagTable_t* TagToTagTableEntry(unsigned short tag)
{
    unsigned int i;
    for (i = 0; i < TAG_TABLE_SIZE; i++) {
        if (TagTable[i].Tag == tag) {
...
static int IsIntegerFormat(int format) {
    if (format == FMT_BYTE ||
        format == FMT_USHORT ||
        format == FMT_ULONG ||
        format == FMT_SBYTE ||
...
int WriteAsStrToDir(int format, int components, char* strValue, char* Buffer, int DirIndex) {

    char* curElement;

    // calculate byteCount
    if (components < 0) {
        curElement = strtok(strValue, ",");
        components = 0;
    ...
    Put32u(Buffer+DirIndex + 4, components);        // component #
    curElement = strtok(strValue, ",");
    int valuePos = 8;

    while (curElement != NULL) {
        if (IsIntegerFormat(format)) {
            int val = atoi(curElement);
            switch (BytesPerFormat[format]) {
                case 1:
                    *(Buffer + DirIndex + valuePos) = val;  break;
                case 2:
                    Put16u(Buffer + DirIndex + valuePos, val); break;
                case 4:
                    Put32u(Buffer + DirIndex + valuePos, val); break;
            }
            valuePos += BytesPerFormat[format];
        } else if (format == FMT_STRING) {
            strncpy(Buffer+DirIndex+8, strValue, byteCount);   // Value (as string)
        } else if (format == FMT_UNDEFINED) {
            memcpy(Buffer+DirIndex+8, strValue, byteCount);    // Value (binary)
        }
        curElement = strtok(NULL, ",");
    }
...
static void writeExifTagAndData(int tag,
...
#ifdef SUPERDEBUG
    printf("writeExifTagAndData(in), tag=0x%04X (%s), IFD=%d format=%d (%s)",
          tag, TagValToName(tag, IFD), IFD, format, formatStr(format));
    printf("writeExifTagAndData(..), DirIndex=%d (0x%X), DataWriteIndex=%d (0x%X)",
          *DirIndex, *DirIndex, *DataWriteIndex,*DataWriteIndex);
#endif
    ...
    if (format == FMT_STRING) {
        ...
        if (components <= 4) {
            ...
            *(Buffer+(*DataWriteIndex)+components) = 0;
    ...
    } else if (format == FMT_UNDEFINED && components > 4) {    // must be written via Pointer?
#ifdef SUPERDEBUG
        printf("processing component format %s, size %ld", formatStr(format), components);
#endif
        Put32u(Buffer+(*DirIndex) + 8, (*DataWriteIndex)-8);   // Pointer
        memcpy(Buffer+(*DataWriteIndex), (char *)value, components);
        (*DataWriteIndex) += components;
    } else if (!WriteAsStrToDir(format, components, (char *)value, Buffer, *DirIndex)) {
        Put32u(Buffer+(*DirIndex) + 8, (*DataWriteIndex)-8);   // Pointer
    ...
    (*DirIndex) += 12;
    if ((*DataWriteIndex) & 1) (*DataWriteIndex)++; // offset shall be even

#ifdef SUPERDEBUG
    printf("writeExifTagAndData(out), DirIndex=%d (0x%X), DataWriteIndex=%d (0x%X)",
           *DirIndex, *DirIndex, *DataWriteIndex,*DataWriteIndex);
#endif
}
...
//--------------------------------------------------------------------------
static void create_EXIF_internal(ExifElement_t* elements, int numElements, char* Buffer)
{
    unsigned short NumEntries;
    int DataWriteIndex;
    int DirIndex;
    int exifDirCount = 0;
    int interOpDirCount = 0;
    int extraDirCount = 2;      // always include the Thumbnail tags
    int exifTagCount = 0;
    int gpsTagCount = 0;

    int gpsTagIndex = 0;
    int thumbnailDirIndex = 0;
    int extraDirIndex = 0;
    int interOpDirIndex = 0;

    {
        int i;
        for (i = 0; i < numElements; i++) {
             switch (elements[i].IFD) {
                case IFD_BASE:
                    exifTagCount++;
                    break;
                case IFD_EXIF:
...
#ifdef SUPERDEBUG
    ALOGE("exifTagCount %d, exifDirCount %d, interOpDirCount %d, extraDirCount %d, gpsTagCount %d",
          exifTagCount, exifDirCount, interOpDirCount, extraDirCount, gpsTagCount);
#endif
    ...
    if (exifDirCount) {
        ++exifTagCount;       // allow for the exif dir info tags
    }
    if (gpsTagCount) {
        ++exifTagCount;       // allow for the GPS info tag
    }
...
#ifdef SUPERDEBUG
        ALOGE("create_EXIF (Basic) NumEntries %d", NumEntries);
#endif

        DataWriteIndex += 2 + NumEntries*12 + 4;
        Put16u(Buffer+DirIndex, NumEntries);
        DirIndex += 2;
        extraDirIndex = DirIndex + NumEntries*12;
        gpsTagIndex   = DirIndex + (NumEntries-1)*12;
        int i;
        for (i = 0; i < numElements; i++) {
            if (elements[i].IFD != IFD_BASE) {
                continue;
            }
#ifdef SUPERDEBUG
            ALOGE("create_EXIF (Basic) DirIndex %d saving tag %x value \"%s\"",
                  DirIndex, elements[i].Tag, elements[i].Value);
#endif
    ...
    if (exifDirCount > 0) {
        ...
        writeExifTagAndData(TAG_EXIF_OFFSET,
         ...
         gpsTagIndex = DirIndex; // save dir position for gps dir

        // move to exifDir
        NumEntries = exifDirCount;
        if (interOpDirCount) {
            NumEntries++;
        }

#ifdef SUPERDEBUG
        ALOGE("create_EXIF (Exif dir) NumEntries %d", NumEntries);
#endif

        DirIndex = DataWriteIndex;
        DataWriteIndex += 2 + NumEntries*12 + 4;

        // write number of components in exifDir
        Put16u(Buffer+DirIndex, NumEntries);
        DirIndex += 2;
        if (extraDirIndex == 0) {
            extraDirIndex = DirIndex + NumEntries*12;
        ...
        // write exifDir tags
        int i;
        int addIopTag = interOpDirCount;
        for (i = 0; i < numElements; i++) {
            if (elements[i].IFD != IFD_EXIF) {
                continue;
            }

            // Interoperability pointer tag shall be inserted sorted
            if (elements[i].Tag > TAG_INTEROP_OFFSET && addIopTag) {
                // Note position and make place for interoperability data
                interOpDirIndex = DirIndex;
                DirIndex += 12;
                addIopTag = FALSE;
            }
#ifdef SUPERDEBUG
            ALOGE("create_EXIF (Exif Dir) DirIndex %d saving tag %x value \"%s\"",
                  DirIndex, elements[i].Tag, elements[i].Value);
#endif
            writeExifTagAndData(elements[i].Tag,
                                elements[i].Format,
    ...
    // *** write interoperability data ***
    if (interOpDirCount) {

        if (interOpDirIndex == 0) {
            interOpDirIndex = DirIndex;
        }
        int interOpDirPtr = DataWriteIndex-8;

        // write link
        writeExifTagAndData(TAG_INTEROP_OFFSET,
                            FMT_ULONG,
                            IFD_INTER_OP,
                            1,
                            interOpDirPtr,
        ...
        // move to inter Op dir
        DirIndex = DataWriteIndex;
        NumEntries = interOpDirCount;

#ifdef SUPERDEBUG
        ALOGE("create_EXIF (Inter op dir) NumEntries %d", NumEntries);
#endif

        // write number of components in inter op dir
        Put16u(Buffer+DirIndex, NumEntries);
        DirIndex += 2;
        DataWriteIndex += 2 + NumEntries*12 + 4;
        ...
        int i;
        for (i = 0; i < numElements; i++) {
            if (elements[i].IFD != IFD_INTER_OP) {
                continue;
            }

#ifdef SUPERDEBUG
            ALOGE("create_EXIF (Inter Op Dir) DirIndex %d saving tag %x value \"%s\"",
                  DirIndex, elements[i].Tag, elements[i].Value);
#endif
            writeExifTagAndData(elements[i].Tag,
                                elements[i].Format,
    ...
    // *** write end of directory data ***
    if (extraDirCount) {
        Put32u(Buffer+extraDirIndex, DataWriteIndex-8);

        // move to extra dir
        DirIndex = DataWriteIndex;
        NumEntries = extraDirCount;
        DataWriteIndex += 2 + NumEntries*12 + 4;

#ifdef SUPERDEBUG
        ALOGE("create_EXIF (end of Dir) NumEntries=%d", NumEntries);
#endif
        ...
        int i;
        for (i = 0; i < numElements; i++) {
            if (elements[i].IFD != IFD_EXTRA) {
                continue;
            }

#ifdef SUPERDEBUG
            ALOGE("create_EXIF (end of Dir) DirIndex %d saving tag %x value \"%s\"",
                  DirIndex, elements[i].Tag, elements[i].Value);
#endif
            writeExifTagAndData(elements[i].Tag,
                                elements[i].Format,
        ...
        thumbnailDirIndex = DirIndex;
    } else {
        // End of directory - no link to continued directory.
        Put32u(Buffer+extraDirIndex, 0);
    }

        // *** write gps data ***
    if (gpsTagCount > 0) {
        int gpsDirPtr = DataWriteIndex-8;
...
#ifdef SUPERDEBUG
        ALOGE("create_EXIF (GPS Dir) NumEntries %d", NumEntries);
#endif
...
#ifdef SUPERDEBUG
            ALOGE("create_EXIF saving GPS tag  DirIndex %d saving tag %x value \"%s\"",
                  DirIndex, elements[i].Tag, elements[i].Value);
#endif
            writeExifTagAndData(elements[i].Tag,
                                entry->Format,
    ...
    // *** Handle thumbnail reference ***

    // Move back to extra dir's thumbnail positions
    DirIndex = thumbnailDirIndex;
    printf("Starting Thumbnail section DirIndex = %d DataWriteIndex %d", DirIndex, DataWriteIndex);
...
void create_EXIF(ExifElement_t* elements, int totNum)
{
#ifdef SUPERDEBUG
    dumpList("create_EXIF", elements, totNum);
#endif
    ...
    if (Buffer != NULL) {
        create_EXIF_internal(elements, totNum, Buffer);
        free(Buffer);
    } else {
        ErrFatal("Could not allocate memory");
    }
...
void ShowImageInfo(int ShowFileInfo)
{
    ...
    if (ImageInfo.DigitalZoomRatio[0]){
        printf("Digital Zoom: %s\n", ImageInfo.DigitalZoomRatio);
    }
    ...
    if (ImageInfo.ExposureTimeJava[0]){
        printf("ExposureTime : %s", ImageInfo.ExposureTimeJava);
    }

    if (ImageInfo.ApertureFNumber){
        printf("Aperture     : f/%s\n", ImageInfo.ApertureFNumberJava);
    }
    ...
    if (ImageInfo.GpsInfoPresent){
        ...
        if (ImageInfo.GpsAltRef >= 0) printf("GPS AltitudeRef : %d\n",ImageInfo.GpsAltRef);
    }
...
void ShowConciseImageInfo(void)
{
    ...
    if (ImageInfo.ExposureTimeJava){
        printf(" %s",ImageInfo.ExposureTimeJava);
    }

    if (ImageInfo.ApertureFNumberJava){
        printf(" %s", ImageInfo.ApertureFNumberJava);
    }
