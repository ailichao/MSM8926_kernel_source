...
static TagTable_t GpsTags[]= {
    ...
    { 0x07, "GPSTimeStamp", FMT_URATIONAL, 3},
...
{
    const TagTable_t* entry = GpsTagToTagTableEntry(tag);
    if (entry != NULL) {
            return entry->Desc;
    }
    return ">>>NotFound";
    ...
    for (de=0;de<NumDirEntries;de++){
...
#ifdef SUPERDEBUG
        printf("GPS tag %x (%s), format %d (%s), #components %d, componentsize %d bytecount %d",
                Tag, GpsTagValToName(Tag), Format, formatStr(Format), Components,
                ComponentSize, ByteCount);
#endif
        ...
        switch(Tag){
            ...
            case TAG_GPS_TIMESTAMP:
                {
                    int hour = Get32u(ValuePtr) /                            // hour numerator
                        (Get32u(ValuePtr + 4) ? Get32u(ValuePtr + 4) : 1);   // hour denominator
                    int minute = Get32u(ValuePtr + 8) /                      // minute numerator
                        (Get32u(ValuePtr + 12) ? Get32u(ValuePtr + 12) : 1); // minute denominator
                    int second = Get32u(ValuePtr + 16) /                     // second numerator
                        (Get32u(ValuePtr + 20) ? Get32u(ValuePtr + 20) : 1); // second denominator
                    snprintf(ImageInfo.GpsTimeStamp,
                             sizeof(ImageInfo.GpsTimeStamp),
                             "%02d:%02d:%02d", hour, minute, second);

                    ImageInfo.GpsTimeStamp[sizeof(ImageInfo.GpsTimeStamp)-1] = 0;   // guard
                    break;
                }
            ...
            case TAG_GPS_PROCESSING_METHOD:
                ...
                break;
            default:
                ReportTagInfo(Tag, Format, Components, ValuePtr, IFD_GPS);
        }
